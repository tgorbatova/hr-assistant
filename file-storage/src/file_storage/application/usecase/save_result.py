...
from file_storage.domain.models.result import ResultId, SaveResult
...
class SaveResultInteractor(SaveResultUseCase):
    ...
    async def save(self, result_info: SaveResultDto, result: BinaryIO) -> ResultId:
        ...
        def _format_path(result_info: SaveResultDto) -> str:
            ...
            :return:
            """
            return f"{settings.INFRA.OBJECT_STORE.RESULT_PATH}/{result_info.folder_name}/{result_info.file_name}/{result_info.type}/{result_info.file_name}_converted.txt"
        ...
        await self._storage.store_file(result, _format_path(result_info), file_size=result_info.size)
